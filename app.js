...
// Loaded common words asynchronously from JSON file
async function loadCommonWords() {
    try {
        const response = await fetch('src/common_wordslist.json');
        if (!response.ok) {
            throw new Error(`Failed to load common words: ${response.statusText}`);
        }
        commonWords = await response.json();
        return true;
    } catch (error) {
        console.error('Error loading common words:', error);
        showError('Failed to load word list. Please try again.');
        return false;
    }
}
...
// Loaded zxcvbn library with promise-based approach
let zxcvbnLoaded = false;
function loadZxcvbn() {
    return new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = 'src/zxcvbn.min.js';
        script.onload = () => {
            zxcvbnLoaded = true;
            resolve();
        };
...
function calculateStrength(password) {
    ...
    try {
        ...
        // Updated progress bar width and color
